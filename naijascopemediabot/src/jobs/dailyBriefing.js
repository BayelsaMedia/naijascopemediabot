...
export async function sendDailyBriefings(fetchRSSItems) {
  ...
  try {
    const res = await query(
      "SELECT u.whatsapp_number, u.language_pref FROM users u JOIN user_subscriptions s ON u.whatsapp_number = s.whatsapp_number WHERE s.subscription_type = 'daily_digest'"
    );
...
export function startDailyBriefingJob(fetchRSSItems) {
  // 7:00 AM Nigeria time (WAT = UTC+1)
