...
async function fetchNews() {
  try {
    ...
    await sendNewsItems(null, items, null);
    return items;
  } catch (err) {
...
function startDailyDigest() {
  setInterval(async () => {
    ...
    if (now.getUTCHours() === 6 && now.getUTCMinutes() < 5 && subscribers.size > 0) {
      console.log(`[DIGEST] Sending daily digest to ${subscribers.size} subscribers`);
      try {
    ...
    try {
      ...
      if (message.type === "interactive") {
        ...
        if (replyId === "btn_news") {
          ...
          await sendMessage(from, "✅ Subscribed! You'll get top headlines every morning at 7AM WAT.\n\nType 'unsubscribe' anytime. Sharp sharp! 📡");
        }
        return;
      }
